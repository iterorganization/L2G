def main():
            ...
            for i in range(n_beams):
                ...
                for scale in isoscales:
                    ...
                    for j in range(n_points):
                        p = points[j]
                        tlas_obj.castRay(p[0], p[1], p[2], k[0], k[1], k[2],
                                           tnear, tfar)
                        total_lines += 1
                        beam_total_rays += 1
                        if not tlas_obj.checkIfHit():
    
                            # Try to see if there is a hit in reverse
                            tlas_obj.castRay(p[0], p[1], p[2], -k[0], -k[1],
                                               -k[2], tnear, tfar)
                            if not tlas_obj.checkIfHit():
                                continue
                        ...
                        beam_hit_counter += 1
                        # Mesh or geometry id.
                        geom_id = tlas_obj.returnGeomId()
                        # Cell or triangle id.
                        primId = tlas_obj.returnPrimId()
    ...
    for file in meshes:
        ...
        geom_id = tlas_obj.commitMesh(vertices, triangles)
